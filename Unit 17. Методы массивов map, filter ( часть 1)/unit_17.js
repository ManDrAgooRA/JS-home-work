...
function t2() {
    ...
    let out = a2.map(function (item, index) {
        a2_res.push(item ** 2);
    })
...
function t9() {
    ...
    b9_num = [];
    b9_string = [];
    let out = b9.filter(function (elem, index) {
        if (typeof elem == 'number') {
            b9_num.push(elem);
        }
    ...
    document.querySelector('.out-9_1').innerHTML = b9_string;
}
...
function t10() {
    let b10 = [[1, 2, 3], [3, 4, 6], [4, 5, 7], [8, 9, 3]]; 4
    let b10_res = b10.filter(function (elem, index) {
        if (elem.indexOf(3) != -1) {
            return true;
        }
    })
